...
int main(int argc, char** argv)
{
	...
	auto bookPrice = [=](Book& tmpBook)
	{
		if (tmpBook.country() == "US")
		{
			return tmpBook.price() *= usdToCadRate;
		}

		else if (tmpBook.country() == "UK")
		{
			if (tmpBook.year() >= 1990 && tmpBook.year() <= 1999)
			{
				return tmpBook.price() *= gbpToCadRate;
			}

			else
			{
				return tmpBook.price();
			}
		}
